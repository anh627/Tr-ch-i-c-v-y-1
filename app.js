...
// Optimized AI move generation with lazy evaluation
const pickAiMove = (board, color, difficulty, koBoard) => {
  const size = board.length;
  const relevantPositions = [];
 
  // Lazy evaluation using generator for position iteration
  ...
  // Process positions lazily
  const positionIterator = generatePositions();
  for (const pos of positionIterator) {
    if (!relevantPositions.some(p => p.x === pos.x && p.y === pos.y)) {
      relevantPositions.push(pos);
    }
    // Early exit for performance
    if (relevantPositions.length >= 50) break;
  }
  ...
  // Lazy evaluation for move scoring
  const candidates = [];
  const scoreIterator = function* () {
    for (const pos of unique) {
      ...
      yield { position: pos, score };
    }
  };

  for (const candidate of scoreIterator()) {
    candidates.push(candidate);
    // Early exit to limit processing
...
const GoGame = () => {
  ...
  const calculateScore = useCallback(() => {
    ...
    return {
      ...
      komi: settings.komi,
      winner: blackScore > whiteScore ? 'black' : whiteScore > whiteScore ? 'white' : 'draw'
    };
  ...
  const handleAIMove = useCallback(() => {
    ...
    aiTimeoutRef.current = setTimeout(() => {
      const aiMove = pickAiMove(deferredBoard, currentPlayer, settings.difficulty, koHistoryRef.current);
  ...
  }, [deferredBoard, currentPlayer, settings.difficulty, gameStatus, getAIColor]);
  ...
  const getGridPosition = useCallback((clientX, clientY) => {
    ...
    const x = Math.round((clientX - rect.left - cellSize / 2) / cellSize);
    const y = Math.round((clientY - rect.top - cellSize / 2) / cellSize);
  ...
  }, [deferredBoard, settings.boardSize, hoverPosition, lastMove, isValidMove, handlePlaceStone, isLoadingAI, animatingCaptures, currentPlayer, getGridPosition]);
